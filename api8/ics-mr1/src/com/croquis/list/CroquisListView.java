...
public class CroquisListView extends CroquisAbsListView {
    ...
    public CroquisListView(Context context, AttributeSet attrs, int defStyle) {
        ...
        CharSequence[] entries = a.getTextArray(
                android.R.styleable.ListView_entries);
        if (entries != null) {
            setAdapter(new ArrayAdapter<CharSequence>(context,
                    android.R.layout.simple_list_item_1, entries));
        }

        final Drawable d = a.getDrawable(android.R.styleable.ListView_divider);
        if (d != null) {
        ...
        final Drawable osHeader = a.getDrawable(
                android.R.styleable.ListView_overScrollHeader);
        if (osHeader != null) {
            setOverscrollHeader(osHeader);
        }

        final Drawable osFooter = a.getDrawable(
                android.R.styleable.ListView_overScrollFooter);
        if (osFooter != null) {
        ...
        mHeaderDividersEnabled = true;//!a.getBoolean(R.styleable.ListView_headerDividersEnabled, true);
        mFooterDividersEnabled = true;//!a.getBoolean(R.styleable.ListView_footerDividersEnabled, true);
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        if (mHeaderViewInfos.size() > 0|| mFooterViewInfos.size() > 0) {
            mAdapter = new CroquisHeaderViewListAdapter(mHeaderViewInfos, mFooterViewInfos, adapter);
        } else {
            mAdapter = adapter;
        }
    ...
    private void setupChild(View child, int position, int y, boolean flowDown, int childrenLeft,
        ...
        CroquisAbsListView.LayoutParams p = (CroquisAbsListView.LayoutParams) child.getLayoutParams();
        if (p == null) {
            p = new CroquisAbsListView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
                    ViewGroup.LayoutParams.WRAP_CONTENT, 0);
        }
        p.viewType = mAdapter.getItemViewType(position);

        if ((recycled && !p.forceAdd) || (p.recycledHeaderFooter &&
                p.viewType == CroquisAdapterView.ITEM_VIEW_TYPE_HEADER_OR_FOOTER)) {
        ...
        /*!
        if (recycled && (((CroquisAbsListView.LayoutParams)child.getLayoutParams()).scrappedFromPosition)
                != position) {
    ...
    private boolean commonKey(int keyCode, int count, KeyEvent event) {
        ...
        if (action != KeyEvent.ACTION_UP) {
            ...
            case KeyEvent.KEYCODE_DPAD_UP:
                ...
                } else if (event.isAltPressed()/*!event.hasModifiers(KeyEvent.META_ALT_ON)!*/) {
            ...
            case KeyEvent.KEYCODE_SPACE:
                if (mPopup == null || !mPopup.isShowing()) {
                    ...
                    } else if (event.isShiftPressed()/*!event.hasModifiers(KeyEvent.META_SHIFT_ON)!*/) {
    ...
    private void scrollListItemsBy(int amount) {
        ...
        if (amount < 0) {
            ...
            while (first.getBottom() < listTop) {
                CroquisAbsListView.LayoutParams layoutParams = (LayoutParams) first.getLayoutParams();
            ...
            while (last.getTop() > listBottom) {
                CroquisAbsListView.LayoutParams layoutParams = (LayoutParams) last.getLayoutParams();
                if (recycleBin.shouldRecycleViewType(layoutParams.viewType)) {
    ...
    public boolean isOpaque() {
        boolean retValue = (mCachingActive && mIsCacheColorOpaque && mDividerIsOpaque &&
                true/*!hasOpaqueScrollbars()!*/) || super.isOpaque();
        if (retValue) {
