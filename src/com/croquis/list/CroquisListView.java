...
public class CroquisListView extends CroquisAbsListView {
	...
	*/
	...
	public CroquisListView(Context context) {
		this(context, null);
	}

	public CroquisListView(Context context, AttributeSet attrs) {
		this(context, attrs, android.R.attr.listViewStyle);
	}

	public CroquisListView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);

		/*
		TypedArray a = context.obtainStyledAttributes(attrs,
				android.R.styleable.ListView, defStyle, 0);

		CharSequence[] entries = a
				.getTextArray(android.R.styleable.ListView_entries);
		if (entries != null) {
		...
		final Drawable d = a
				.getDrawable(android.R.styleable.ListView_divider);
		if (d != null) {
		...
		final int dividerHeight = a.getDimensionPixelSize(
				android.R.styleable.ListView_dividerHeight, 0);
		if (dividerHeight != 0) {
			setDividerHeight(dividerHeight);
		}
		*/

		//setChoiceMode(a.getInt(R.styleable.ListView_choiceMode,
		//		CHOICE_MODE_NONE));

		mHeaderDividersEnabled = true;//a.getBoolean(
				//R.styleable.ListView_headerDividersEnabled, true);
		mFooterDividersEnabled = true;//a.getBoolean(
	...
	public void setAdapter(ListAdapter adapter) {
		...
		if (mHeaderViewInfos.size() > 0 || mFooterViewInfos.size() > 0) {
			mAdapter = new CroquisHeaderViewListAdapter(mHeaderViewInfos,
	...
	void fillGap(boolean down) {
		...
		if (down) {
			final int startOffset = count > 0 ? getChildAt(count - 1)
					.getBottom() + mDividerHeight : getListPaddingTop();
	...
	private View moveSelection(View oldSel, View newSel, int delta,
		...
		final int topSelectionPixel = getTopSelectionPixel(childrenTop,
				fadingEdgeLength, selectedPosition);
		final int bottomSelectionPixel = getBottomSelectionPixel(childrenTop,
				fadingEdgeLength, selectedPosition);
	...
	final int measureHeightOfChildren(int widthMeasureSpec, int startPosition,
		...
		endPosition = (endPosition == NO_POSITION) ? adapter.getCount() - 1
				: endPosition;
		final CroquisAbsListView.RecycleBin recycleBin = mRecycler;
	...
	private void setupChild(View child, int position, int y, boolean flowDown,
		...
		CroquisAbsListView.LayoutParams p = (CroquisAbsListView.LayoutParams) child
				.getLayoutParams();
		if (p == null) {
			p = new CroquisAbsListView.LayoutParams(
		...
		if ((recycled && !p.forceAdd)
				|| (p.recycledHeaderFooter && p.viewType == CroquisAdapterView.ITEM_VIEW_TYPE_HEADER_OR_FOOTER)) {
			attachViewToParent(child, flowDown ? -1 : 0, p);
		} else {
			p.forceAdd = false;
			if (p.viewType == CroquisAdapterView.ITEM_VIEW_TYPE_HEADER_OR_FOOTER) {
	...
	private int amountToScroll(int direction, int nextSelectedPosition) {
		...
		if (direction == View.FOCUS_DOWN) {
			...
			if ((mFirstPosition + numChildren) == mItemCount) {
				...
				final int max = getChildAt(numChildren - 1).getBottom()
						- listBottom;
			...
			if (direction == View.FOCUS_DOWN) {
				...
				final int listTop = mListPadding.top
						+ (topFadingEdgeShowing ? getArrowScrollPreviewLength()
								: 0);
				final int ySearchPoint = (selectedView != null && selectedView
						.getTop() > listTop) ? selectedView.getTop() : listTop;
				mTempRect.set(0, ySearchPoint, 0, ySearchPoint);
			} else {
				final boolean bottomFadingEdgeShowing = (mFirstPosition
				...
				final int listBottom = getHeight()
						...
						- (bottomFadingEdgeShowing ? getArrowScrollPreviewLength()
								: 0);
				final int ySearchPoint = (selectedView != null && selectedView
						.getBottom() < listBottom) ? selectedView.getBottom()
						: listBottom;
				mTempRect.set(0, ySearchPoint, 0, ySearchPoint);
			}
	...
	private void scrollListItemsBy(int amount) {
		...
		if (amount < 0) {
			...
			while (first.getBottom() < listTop) {
				CroquisAbsListView.LayoutParams layoutParams = (LayoutParams) first
						.getLayoutParams();
			...
			while (last.getTop() > listBottom) {
				CroquisAbsListView.LayoutParams layoutParams = (LayoutParams) last
						.getLayoutParams();
				if (recycleBin.shouldRecycleViewType(layoutParams.viewType)) {
	...
	public boolean isOpaque() {
		return (mCachingStarted && mIsCacheColorOpaque && mDividerIsOpaque/* && hasOpaqueScrollbars()*/)
				|| super.isOpaque();
	}
	...
	View findViewTagInHeadersOrFooters(ArrayList<FixedViewInfo> where,
			Object tag) {
		if (where != null) {
			int len = where.size();
			View v;
	...
	*/
	...
	static class SavedState extends BaseSavedState {
		...
		public void writeToParcel(Parcel out, int flags) {
			...
			out.writeLongArray(checkIdState != null ? checkIdState.getKeys()
					: new long[0]);
		}
